module MCPTools
  class StatsNewAdozioni < Base
    ...
    description "Statistiche adozioni dal database nazionale MIUR aggiornato (miur_adozioni/miur_scuole — dati pubblici, non legati al tuo account). Stessa interfaccia di stats_adozioni ma interroga le tabelle miur_adozioni e miur_scuole, aggiornate ad ogni sync MIUR. Di default interroga tutti i gradi (elementari + medie + superiori); restringi con il filtro `grado` (E=elementari, M=medie, N=superiori, virgola-separati). Il filtro `filiera` (liceo|tecnico|professionale|altro, virgola-separati) distingue le scuole superiori per tipologia. Filtra per area (NORD OVEST|NORD EST|CENTRO|SUD|ISOLE), regione, provincia, comune, classe, editore, disciplina, titolo, isbn. Il filtro `nuova_adozione` (true/false) isola i libri adottati nuovi quest'anno nella scuola (NUOVAADOZ del MIUR): rilevante per medie/superiori, dove le nuove adozioni non hanno mercato dell'usato. Aggrega con group_by: editore, disciplina, classe, area, regione, provincia, comune, titolo, isbn (source of truth), scuola, grado, tipo_scuola, filiera, nuova_adozione. La provincia accetta sia il nome completo (PRATO) che la sigla (PO). Con `include_sezioni: true` ogni riga riporta anche l'array delle sezioni coinvolte (es. \"5A [C]\") — utile soprattutto per l'analisi di una singola scuola."
