...
class CommandMenuComponent < ViewComponent::Base
  ...
  def initialize(id: "command_menu", enabled: true, theme: "light", show_key: "k", placeholder: "Go to page…", max_container_width: "max-w-2xl", collapse_on_open: true)
    @id = id
    @enabled = enabled
    @theme = theme
    @show_key = show_key
  ...
  def container_data
    {
      ...
      command_menu_filtering_value: false,
  ...
  def command_menu_css
    class_names(
      ...
      "z-10",
      {
        "text-gray-700 bg-white ring-gray-100 focus-within:ring-gray-200": light_theme?,
        "text-gray-300 bg-gray-800 ring-gray-900 focus-within:border-gray-600/60": dark_theme?
      }
    )
  end
  ...
  def input_field
    tag.input data: {command_menu_target: "input", action: "input->command-menu#filter"},
      ...
      class: class_names(
        ...
        {
          "placeholder:text-gray-400": light_theme?,
          "placeholder:text-text-gray-400": dark_theme?
        }
      )
  end
  ...
  def grouped_items_list(group, items)
    tag.li class: class_names("px-1 group-data-[command-menu-list-open-value=true]/container:first:border-t", {"border-gray-100": light_theme?, "border-gray-700": dark_theme?}) do
      capture do
        concat tag.p group.humanize, class: class_names("mt-2 px-3 text-xs font-semibold", {"text-gray-500": light_theme?, "text-gray-400": dark_theme?}) if group.present? && !@collapse_on_open
  ...
  def icon
    ...
    <<-SVG.html_safe
      <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="size-5 ml-4 shrink-0 opacity-60 block group-data-[command-menu-filtering-value=true]/container:hidden">
  ...
  def footer_css
    class_names(
      ...
      {
        "text-gray-600 bg-gray-50 border-gray-100": light_theme?,
        "text-gray-400 bg-gray-900 border-gray-700": dark_theme?
      }
    )
  end
