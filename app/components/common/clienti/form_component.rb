...
module Common
  class Clienti::FormComponent < Common::MultistepFormComponent
    ...
    def call
      tag.div(super, class: "font-primary xxxmin-w-[700px] mx-auto gap-4")
    end
          ...
          tag.div(
            ...
            concat(
              tag.div(class: "mt-5 w-full inline-flex gap-2 items-center justify-end") do
                concat(link_to("annulla", back_url, class: "px-4 py-2 bg-gray-500 text-white")) if index == 0
                if index > 0
                  concat(tag.label("indietro", for: radio_id(index - 1), class: "px-4 py-2 bg-gray-500 text-white"))
                end
                if index < (steps.count - 1)
                  concat(form.submit("avanti", class: "cursor-pointer px-4 py-2 bg-gray-500 text-white"))
                end
                if index == (steps.count - 1)
                  concat(form.submit("salva", class: "cursor-pointer px-4 py-2 bg-gray-500 text-white"))
                end
              end
            )
          end
