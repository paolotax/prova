...
module Persona::Avatar
  ...
  def avatar_content_type_allowed
    unless ALLOWED_AVATAR_CONTENT_TYPES.include?(avatar.content_type)
      errors.add(:avatar, "deve essere un'immagine JPEG, PNG, GIF o WebP")
    end
  end
  ...
  def avatar_dimensions_allowed
    ...
    if width && width > MAX_AVATAR_DIMENSIONS[:width]
      errors.add(:avatar, "larghezza deve essere inferiore a #{MAX_AVATAR_DIMENSIONS[:width]}px")
    end

    if height && height > MAX_AVATAR_DIMENSIONS[:height]
      errors.add(:avatar, "altezza deve essere inferiore a #{MAX_AVATAR_DIMENSIONS[:height]}px")
    end
  end
end
