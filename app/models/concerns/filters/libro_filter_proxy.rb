module Filters
  module LibroFilterScopes
    ...
    filter_scope :categoria, ->(name) { joins(:categoria).where("categorie.nome_categoria ILIKE ?", "%#{name}%") }
    ...
    filter_scope :incompleti, ->(incompleti) { incompleti == "si" ? where("libri.editore_id IS NULL OR libri.categoria_id IS NULL") : all }
