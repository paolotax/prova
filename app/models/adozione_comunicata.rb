  ...
  def self.importa_da_excel(file_path, user)
    ...
    (2..xlsx.last_row).each do |row_num|
      begin
        ...
        # Gestisce la descrizione scuola: se vuota, cerca di popolarla dalla scuola collegata
        descrizione_scuola = row_data['Descrizione']
        if descrizione_scuola.nil? || descrizione_scuola.to_s.strip.empty?
          # Cerca la scuola corrispondente per popolare la descrizione
        ...
        # Gestisce l'editore: se vuoto, cerca di popolarlo da import_adozione
        editore = row_data['Editore']
        if editore.nil? || editore.to_s.strip.empty?
          # Cerca un'adozione corrispondente per popolare l'editore
          adozione_corrispondente = ImportAdozione.where(CODICEISBN: row_data['Ean'])
                                                  .where(CODICESCUOLA: row_data['CodMinisteriale'])
                                                  .first
          if adozione_corrispondente
            editore = adozione_corrispondente.EDITORE
          end
        end
        ...
        # Se la descrizione scuola era ancora vuota dopo la creazione/aggiornamento, prova a popolarla dalla corrispondenza trovata
        if adozione.descrizione_scuola.nil? || adozione.descrizione_scuola.strip.empty?
          if adozione.import_adozione&.import_scuola&.DENOMINAZIONESCUOLA
            adozione.update!(descrizione_scuola: adozione.import_adozione.import_scuola.DENOMINAZIONESCUOLA)
          end
        end
        
        # Se l'editore era ancora vuoto dopo la creazione/aggiornamento, prova a popolarlo dalla corrispondenza trovata
        if adozione.editore.nil? || adozione.editore.strip.empty?
          if adozione.import_adozione&.EDITORE
            adozione.update!(editore: adozione.import_adozione.EDITORE)
          end
        end
      rescue => e
        errori += 1
