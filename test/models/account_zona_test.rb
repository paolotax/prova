...
class AccountZonaTest < ActiveSupport::TestCase
  ...
  test "validates provincia presence" do
    zona = AccountZona.new(account: @fizzy, grado: "E", anno_scolastico: "2025/2026")
    assert_not zona.valid?
    assert zona.errors[:provincia].any?
  end

  test "validates grado presence" do
    zona = AccountZona.new(account: @fizzy, provincia: "TO", anno_scolastico: "2025/2026")
    assert_not zona.valid?
  ...
  test "scopes to current account" do
    fizzy_zone = AccountZona.for_account(@fizzy)
    acme_zone = AccountZona.for_account(@acme)
