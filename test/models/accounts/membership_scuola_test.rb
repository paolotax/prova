...
class Accounts::MembershipScuolaTest < ActiveSupport::TestCase
  ...
  test "validates uniqueness of scuola per membership" do
    ...
    duplicate = Accounts::MembershipScuola.new(membership: existing.membership, scuola: existing.scuola)
  ...
  test "destroying membership destroys membership_scuole" do
    membership = memberships(:bob_fizzy)
    assert_difference("Accounts::MembershipScuola.count", -1) do
      membership.destroy!
    end
  end
end
