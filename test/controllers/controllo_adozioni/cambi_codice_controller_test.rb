...
class ControlloAdozioni::CambiCodiceControllerTest < ActionDispatch::IntegrationTest
  ...
  test "create accoda il fan-out per l'admin, scoped alla provincia" do
    assert_enqueued_with(job: AggiornaCambiCodiceJob,
                         args: [@account, { provincia: "MI" }]) do
      post controllo_adozioni_cambi_codice_path(account_id: @account.id, provincia: "MI")
    end
  ...
  test "create senza provincia accoda account-wide" do
    assert_enqueued_with(job: AggiornaCambiCodiceJob,
      ...
      post controllo_adozioni_cambi_codice_path(account_id: @account.id)
    end
  end

  test "create vietata ai member" do
    sign_in_as(users(:two), @account)
    assert_no_enqueued_jobs only: AggiornaCambiCodiceJob do
      post controllo_adozioni_cambi_codice_path(account_id: @account.id)
    end
